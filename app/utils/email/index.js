...
const { CRYPTO_SENDGRID_API_KEY, CRYPTO_HOST_URL, CRYPTO_SENDGRID_EMAIL } = config;
...
sgMail.setApiKey(CRYPTO_SENDGRID_API_KEY);
...
class Email {
  ...
  static async sendVerificationMail(options) {
    const link = `${CRYPTO_HOST_URL}/login?token=${options.token}`;
    ...
    const message = {
      to: options.email,
      from: CRYPTO_SENDGRID_EMAIL, // Use the email address or domain you verified above
      subject: 'Welcome to cryto wallet',
      html: emailTemplate
    };
  ...
  static async sendResetPasswordEmail(options) {
    const link = `${CRYPTO_HOST_URL}/reset-password?token=${options.token}`;
    const mailGen = configMailGen();
    const emailFormat = await resetPasswordMailGenFormat(link);
    const emailTemplate = mailGen.generate(emailFormat);
    const message = {
      to: options.email,
      from: CRYPTO_SENDGRID_EMAIL,
